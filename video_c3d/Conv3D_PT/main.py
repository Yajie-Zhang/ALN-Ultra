...
def evaluation(model,data_loader,args):
    ...
    with torch.no_grad():
        for i, (index, label, video_value, image_values) in enumerate(data_loader):
            ...
            raw_logits, _ = model(video_value,image_values)
...
def train(train_loader,test_loader,args):
    ...
    model = CNN3D(t_dim=args.n_frame, img_x=args.img_size[0], img_y=args.img_size[1],
                  drop_p=0.2, fc_hidden1=256, fc_hidden2=256, num_classes=args.num_classes).to(device)
    # state_dict = torch.load(
    #     '/home/yjzhang/code/Fujian_Video/video_pretrain/pre_train_model',
    #     map_location='cpu')
    ...
    for epoch in range(args.epoch):
        ...
        for i_, (index, label, video_value, image_values) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            output,output_image = model(video_value,image_values)
            loss = criterion(output, label)#+criterion(output_image,label)
