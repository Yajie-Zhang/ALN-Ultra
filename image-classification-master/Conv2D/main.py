...
def train(train_loader,test_loader,args):
    device=torch.device(args.device)
    # create model
    model = CNN2D(img_x=args.img_size[0], img_y=args.img_size[1], num_classes=args.num_classes)
    model.to(device)
...
def read_excel(key_list, path):
    import pandas as pd
    df = pd.read_excel(path, sheet_name='Sheet1')  # 可以指定工作表名
    age=df['年龄'].values
    size=df['超声测量肿瘤大小'].values
    length=df['肿瘤最长径'].values
    key_excel=df['匿名-key'].values
    sub_label=df['组织类型（浸润导管癌，浸润小叶癌，其他）'].values

    data_all=[]
    for key in key_excel:
        if str(key) in key_list:
            cur_id=np.where(key_excel==key)[0]
            cur_age=age[cur_id][0]
            cur_size=(size[cur_id][0]).split('×')
            cur_size0=cur_size[0].replace('mm','')
            cur_size1=cur_size[1].replace('mm','')
            cur_length=(length[cur_id][0]).replace('mm','')
            cur_size0=float(cur_size0)
            cur_size1=float(cur_size1)
            cur_length=float(cur_length)
            cur_sublabel=sub_label[cur_id]
            # if cur_sublabel=='浸润性导管癌':
            #     cur_sublabel=0
            # elif cur_sublabel=='浸润性癌':
            #     cur_sublabel=1
            #     elif cur_sublabel==''
            cur_data=[cur_age,cur_size0,cur_size1,cur_length]
...
def main(args):
    ...
    excel_path = 'attribute.xlsx'
    att_list = read_excel(key_list, excel_path)
...
if __name__ == '__main__':
    ...
    print(mean_results)
    np.save('/home/yjzhang/code/Fujian_Video/results/image_conv2d_1frame_128.npy',results_all_seed)
