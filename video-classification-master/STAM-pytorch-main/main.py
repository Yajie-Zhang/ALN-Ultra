...
def train(train_loader,test_loader,args):
    ...
    model = STAM(
        dim=512,
        image_size=args.img_size,  # size of image
        patch_size=(16, 20),  # patch size
        num_frames=args.n_frame,  # number of image frames, selected out of video
        space_depth=2,  # depth of vision transformer
        space_heads=4,  # heads of vision transformer
...
if __name__ == '__main__':
    ...
    print(mean_results)
    np.save('/home/yjzhang/code/Fujian_Video/results/video_vvt_'+str(args.n_frame)+'frame_'+str(args.img_size[0])+'.npy', results_all_seed)